...
class Model(nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """
        return self.predict(source, stream, diversity, **kwargs)
    ...
    def predict(
        ...
        **kwargs,
        ...
        return self.predictor.predict_cli(source=source, diversity = diversity) if is_cli else self.predictor(source=source, stream=stream, diversity = diversity)
    ...
    def return_dataset(
        ...
        **kwargs,
        ...
        self.trainer.hub_session = self.session  # attach optional HUB session
        train_dataset = self.trainer.get_data()
        # Update model and cfg after training
        # if RANK in {-1, 0}:
        #     ckpt = self.trainer.best if self.trainer.best.exists() else self.trainer.last
        #     self.model, _ = attempt_load_one_weight(ckpt)
